...
#TODO: <g></g> <b/>
lt-expand .deps/apertium-hbs.hbs.dix  | grep -v 'REGEX' | grep -v '\/' | grep -v '\+' | grep -v ':<:' | LC_ALL=C sed 's/:>:/:/g' | cut -f2 -d':' | LC_ALL=C sed 's/^/^/g' | LC_ALL=C sed 's/$/$/g' | apertium-pretransfer | lt-proc -b $TMF/hbs_HR-hbs_SR.bin > $TMF/biltrans
...
lt-expand .deps/apertium-hbs.hbs.dix  | grep -v 'REGEX' | grep -v '\/' | grep -v '\+' | grep -v ':<:' | LC_ALL=C sed 's/:>:/:/g' | cut -f2 -d':' | LC_ALL=C sed 's/^/^/g' | LC_ALL=C sed 's/$/$/g' | apertium-pretransfer | lt-proc -b $TMF/hbs_SR-hbs_HR.bin > $TMF/biltrans
cat $TMF/biltrans | grep '@' | cut -f1 -d'/' | LC_ALL=C sed 's/\^//g' | sort -u |  python expand-to-dix.py | LC_ALL=C sed 's/&/&amp;/g'  | sort -u >$TMF/dix
