...
#TODO: <g></g> <b/>
lt-expand .deps/apertium-hbs.hbs.dix  | grep -v 'REGEX' | grep -v '\/'  | grep -v ':<:' | LC_ALL=C sed 's/:>:/:/g;' | cut -f2 -d':' | LC_ALL=C sed 's/^/^/g;' | LC_ALL=C sed 's/$/$/g;' | apertium-pretransfer  | sed -r 's:\$[ ]*\^:$\n^:g;'  | lt-proc -b $TMF/hbs_HR-hbs_SR.bin > $TMF/biltrans
